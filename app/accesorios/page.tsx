...
export default function RegistroAccesorios() {
  ...
  const cargarDatos = async () => {
    setLoading(true)
    const { data } = await supabase.from('accesorios').select('*').order('id', { ascending: false })
    setItems(data || [])
    setLoading(false)
  }
  ...
  const guardar = async () => {
    if (!form.codigo_accesorio || !form.detalle) return alert('Código y Detalle son obligatorios')
    ...
    if (editando) {
      await supabase.from('accesorios').update(payload).eq('id', editando.id)
    } else {
  ...
  const resetForm = () => setForm({ codigo_accesorio: '', detalle: '', medidas: '', precio_compra: '', precio_cotizador: '', proveedor: '' })

  const itemsFiltrados = items.filter(i => 
    (i.detalle || '').toLowerCase().includes(busqueda.toLowerCase()) || 
    (i.codigo_accesorio || '').toLowerCase().includes(busqueda.toLowerCase())
  )

  const inputS: any = { padding: '12px', borderRadius: '8px', border: '1px solid #ddd', width: '100%', fontSize: '16px', marginBottom: '10px' }

  if (loading) return <p style={{ textAlign: 'center', marginTop: '50px' }}>Cargando Accesorios...</p>
    ...
    <div style={{ fontFamily: 'Arial, sans-serif', backgroundColor: '#f5f5f5', minHeight: '100vh' }}>
      ...
      <div style={{ padding: '15px', maxWidth: '1100px', margin: '0 auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
          <h2 style={{ margin: 0, fontSize: '20px' }}>⚙️ Registro de Accesorios</h2>
          <button 
            onClick={() => { resetForm(); setEditando(null); setModal(true) }} 
            style={{ padding: '10px 20px', backgroundColor: '#087e0b', color: 'white', border: 'none', borderRadius: '8px', fontWeight: 'bold' }}
          >
            + Adicionar
          </button>
        </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: '600px' }}>
            <thead>
              <tr style={{ backgroundColor: '#f9f9f9', borderBottom: '2px solid #eee' }}>
                ...
                <th style={{ padding: '12px', textAlign: 'right', fontSize: '13px' }}>P. Compra</th>
            ...
            <tbody>
              {itemsFiltrados.map(item => (
                <tr key={item.id} style={{ borderBottom: '1px solid #eee' }}>
                  <td style={{ padding: '12px', fontWeight: 'bold', fontSize: '14px' }}>{item.codigo_accesorio}</td>
                  <td style={{ padding: '12px', fontSize: '14px' }}>
                    {item.detalle} <br/>
                    <small style={{color:'#666'}}>{item.medidas || 'Sin medidas'}</small>
                  </td>
                  <td style={{ padding: '12px', textAlign: 'right', fontSize: '14px' }}>{Number(item.precio_compra).toFixed(2)}</td>
                  <td style={{ padding: '12px', textAlign: 'right', fontWeight: 'bold', color: '#087e0b', fontSize: '14px' }}>
                    Bs. {Number(item.precio_cotizador).toFixed(2)}
                  </td>
      ...
      {modal && (
        <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 2000, padding: '10px' }}>
          <div style={{ backgroundColor: 'white', padding: '20px', borderRadius: '15px', width: '100%', maxWidth: '500px', maxHeight: '90vh', overflowY: 'auto' }}>
            <h3 style={{ marginTop: 0 }}>{editando ? 'Editar Accesorio' : 'Nuevo Accesorio'}</h3>
            
            <label style={{ fontSize: '12px', color: '#666' }}>Código del Accesorio</label>
            <input value={form.codigo_accesorio || ''} onChange={e => setForm({...form, codigo_accesorio: e.target.value})} style={inputS} />
            
            <label style={{ fontSize: '12px', color: '#666' }}>Detalle / Descripción</label>
            <input value={form.detalle || ''} onChange={e => setForm({...form, detalle: e.target.value})} style={inputS} />
            
            <label style={{ fontSize: '12px', color: '#666' }}>Medidas</label>
            <input value={form.medidas || ''} onChange={e => setForm({...form, medidas: e.target.value})} style={inputS} />
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
              <div style={{ flex: 1 }}>
                <label style={{ fontSize: '12px', color: '#666' }}>Precio Compra</label>
            ...
            <div style={{ display: 'flex', gap: '10px', marginTop: '10px' }}>
              ...
              <button onClick={guardar} style={{ flex: 1, padding: '12px', backgroundColor: '#087e0b', color: 'white', border: 'none', borderRadius: '8px', fontWeight: 'bold' }}>Guardar</button>
            </div>
          </div>
        </div>
      )}
